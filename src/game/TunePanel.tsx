...
const truckFields: Field<TruckTunable>[] = [
  ...
  { key: 'topSpeedTarget',     min: 5,      max: 80,      step: 0.5,
    tip: 'Speed (m/s) at which torque drops to zero. Hard cap on top speed.\nCurve = 1 − (v/top)^torqueExponent.' },
  { key: 'torqueExponent',     min: 0.1,    max: 6,       step: 0.05,
    tip: 'Torque curve shape: scale = 1 − (v/top)^p.\n<1 = sheds force fast (aggressive launch, long crawl to top).\n  0.5 = sqrt drop, slow tail.\n  0.3 = even more aggressive launch, even longer tail.\n=1 = straight linear ramp.\n>1 = plateau low, sharp drop near top (faster overall).\nLower for slower top-speed runs at same peakTorque.' },
  ...
  { key: 'maxSteerHighSpeed',  min: 0.02,   max: 0.6,     step: 0.01,
    tip: 'Front wheel angle (rad) at/above steerSpeedRef. Down = stable highway.\nTry 0.05 (~3°) so highway feels calm.' },
  { key: 'steerSpeedRef',      min: 5,      max: 80,      step: 1,
    tip: 'Speed (m/s) at which steering reaches maxSteerHighSpeed.\nDropoff curve from 0→ref governed by steerExponent.' },
  { key: 'steerExponent',      min: 0.1,    max: 6,       step: 0.05,
    tip: 'Steer curve shape: retention = 1 − (v/ref)^p.\n<1 = drops sharply at low speeds, stays near min.\n=1 = linear lerp.\n>1 = stays near maxSteer at low speeds, progressive drop near ref.' },
